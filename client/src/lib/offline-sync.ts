...
interface SyncQueueItem {
    id: string;
    type: 'order' | 'product' | 'customer' | 'discount' | 'settings' | 'category';
...
class OfflineSync {
    private dbPromise: Promise<IDBPDatabase<POSDatabase>> | null = null;
    ...
    private async getDb(): Promise<IDBPDatabase<POSDatabase>> {
        if (!this.dbPromise) {
    ...
    async queueAction(type: SyncQueueItem['type'], action: SyncQueueItem['action'], data: any) {
        ...
        if (typeof navigator !== 'undefined' && navigator.onLine) {
            try {
                ...
                console.warn('Failed to process sync queue after queuing action:', error);
            }
        }
            ...
            for (const item of queue) {
                try {
                    ...
                    if (item.retries >= 3) {
                        ...
                        // Update retries in DB
                        try {
                            await db.put('syncQueue', item);
                        } catch (updateError) {
                            console.warn('Failed to update retry count for sync item:', updateError);
                        }
                    }
                }
            }
    ...
    private async syncItem(item: SyncQueueItem) {
        // Check if we're in Electron and can use direct database access
        if (window.electronAPI) {
            ...
            switch (type) {
                case 'order':
                    if (action === 'create') {
                        await electronAPI.createOrder(data);
                    }
                    break;

                case 'product':
                    if (action === 'create') {
                        await electronAPI.createProduct(data);
                    } else if (action === 'update') {
                        await electronAPI.updateProduct(data.id, data);
                    } else if (action === 'delete') {
                        await electronAPI.deleteProduct(data.id);
                    }
                    break;

                case 'customer':
                    if (action === 'create') {
                        await electronAPI.createCustomer(data);
                    }
                    break;

                case 'category':
                    if (action === 'create') {
                        await electronAPI.createCategory(data);
                    }
                    break;

                case 'discount':
                    // Discounts are typically read-only, so no sync needed
                    break;

                case 'settings':
                    if (action === 'update') {
                        await electronAPI.updateSettings(data);
                    }
                    break;
            }
        } else {
