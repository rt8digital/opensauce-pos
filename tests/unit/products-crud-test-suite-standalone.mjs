/**
 * Self-Contained Products CRUD Test Suite
 * Tests all CRUD operations, edge cases, validation, and performance for the Products entity
 * This version is self-contained and doesn't depend on external schema imports
 */
...
class ProductsCRUDTestSuite {
    ...
    async initialize() {
        ...
        const schema = { products: null, categories: null };

        // Create simple table definitions for drizzle
        const productsTable = {
            _: { name: 'products' },
            id: { name: 'id' },
            name: { name: 'name' },
        ...
        schema.categories = categoriesTable;
    ...
    async setupSchema() {
        ...
        for (const cat of categories) {
            ...
            ).run(cat.name, cat.description, Date.now());
        }
    ...
    async testCreateValidProduct() {
        ...
        try {
            const result = this.db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run(product.name, product.price, product.cost, product.image, product.stockQuantity, product.barcode, product.category);

            if (result.lastInsertRowid) {
                // Verify the created product
                const retrieved = this.db.prepare('SELECT * FROM products WHERE id = ?').get(result.lastInsertRowid);
        ...
        try {
            ...
            const insertMany = this.db.transaction((products) => {
                for (const product of products) {
                    stmt.run(product.name, product.price, product.cost, product.image, product.stockQuantity, product.barcode, product.category);
                }
            });
    ...
    async testCreateProductWithCategory() {
        try {
            ...
            const catResult = this.db.prepare(
                'INSERT INTO categories (name, description, created_at) VALUES (?, ?, ?)'
            ).run('Test Category', 'Test category for products', Date.now());
    ...
    async testReadSingleProduct() {
        try {
            ...
            const result = this.db.prepare(`
                INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run('Read Test Product', '25.99', '12.99', 'read-test.jpg', 75, 'READTEST123', 'General');
    ...
    async testReadAllProducts() {
        try {
            ...
            for (const product of products) {
                stmt.run(product.name, product.price, '0', product.image, product.stockQuantity, product.barcode, 'General');
            }

            // Read all products
            const allProducts = this.db.prepare('SELECT * FROM products').all();
    ...
    async testReadProductsWithFiltering() {
        try {
            ...
            const appleProducts = this.db.prepare('SELECT * FROM products WHERE name LIKE ?').all('%Apple%');
    ...
    async testSearchByBarcode() {
        try {
            ...
            const found = this.db.prepare('SELECT * FROM products WHERE barcode = ?').get(testBarcode);
    ...
    async testSearchByPLU() {
        try {
            ...
            const found = this.db.prepare('SELECT * FROM products WHERE plu = ?').get(testPLU);
    ...
    async testUpdateProduct() {
        try {
            ...
            const result = this.db.prepare(`
                INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run('Original Product', '10.00', '5.00', 'original.jpg', 10, 'UPDATE_TEST_123', 'General');
            ...
            const updateResult = this.db.prepare(`
                ...
                WHERE id = ?
            `).run('Updated Product', '15.00', 20, result.lastInsertRowid);
            ...
            // Verify the update
            const updated = this.db.prepare('SELECT * FROM products WHERE id = ?').get(result.lastInsertRowid);
    ...
    async testPartialUpdate() {
        try {
            const result = this.db.prepare(`
                INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run('Partial Update Test', '12.00', '6.00', 'partial.jpg', 15, 'PARTIAL_UPDATE_123', 'General');
            ...
            const updateResult = this.db.prepare(`
                ...
                SET name = ?
                WHERE id = ?
            `).run('Partially Updated Product', result.lastInsertRowid);
    ...
    async testUpdateWithCategoryReference() {
        try {
            ...
            const catResult = this.db.prepare(
                'INSERT INTO categories (name, description, created_at) VALUES (?, ?, ?)'
            ).run('Update Test Category', 'Category for testing updates', Date.now());
            ...
            const result = this.db.prepare(`
                INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run('Category Update Test', '20.00', '10.00', 'cat-update.jpg', 25, 'CAT_UPDATE_123', 'General');
            ...
            const updateResult = this.db.prepare(`
                ...
                WHERE id = ?
            `).run(catResult.lastInsertRowid, 'Update Test Category', result.lastInsertRowid);
    ...
    async testDeleteProduct() {
        try {
            ...
            const result = this.db.prepare(`
                INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run('Product to Delete', '30.00', '15.00', 'delete-test.jpg', 5, 'DELETE_TEST_123', 'General');

            // Delete the product
            const deleteResult = this.db.prepare('DELETE FROM products WHERE id = ?').run(result.lastInsertRowid);

            // Verify deletion
            const found = this.db.prepare('SELECT * FROM products WHERE id = ?').get(result.lastInsertRowid);
    ...
    async testDeleteNonExistentProduct() {
        try {
            const deleteResult = this.db.prepare('DELETE FROM products WHERE id = ?').run(99999);
    ...
    async testProductWithSpecialCharacters() {
        try {
            ...
            if (result.lastInsertRowid) {
                const created = this.db.prepare('SELECT * FROM products WHERE id = ?').get(result.lastInsertRowid);
    ...
    async testProductWithNullFields() {
        try {
            ...
            const result = this.db.prepare(`
                INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run(product.name, product.price, null, product.image, product.stockQuantity, product.barcode, product.category);
            ...
            this.logIssue(`EDGE CASE: Failed to handle null fields - ${error.message}`);
            throw error;
        }
    }
    ...
    async testSearchPerformance() {
        try {
            ...
            const startTime = Date.now();
            const results = this.db.prepare('SELECT * FROM products WHERE name LIKE ?').all(searchTerm);
            const endTime = Date.now();
    ...
    async testDatabaseConstraintViolations() {
        ...
        try {
            ...
            this.db.prepare(`
                INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).run(product1.name, product1.price, '0', product1.image, product1.stockQuantity, product1.barcode, product1.category);

            try {
                this.db.prepare(`
                    INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `).run(product2.name, product2.price, '0', product2.image, product2.stockQuantity, product2.barcode, product2.category);
            } catch (error) {
            ...
            try {
                this.db.prepare(`
                    INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category_id, category)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                `).run('FK Violation Test', '20.00', '10.00', 'fk-violation.jpg', 10, 'FK_VIOLATION_123', 99999, 'General');
            } catch (error) {
    ...
    async testInvalidDataFormats() {
        try {
            ...
            // Test 1: Invalid price format (should work as TEXT in SQLite)
            try {
                this.db.prepare(`
                    INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `).run('Invalid Price Test', 'not-a-number', '0', 'invalid-price.jpg', 10, 'INVALID_PRICE_123', 'General');
            } catch (error) {
                formatErrors.push('invalid_price');
            }

            // Test 2: Invalid stock quantity (should fail)
            try {
                this.db.prepare(`
                    INSERT INTO products (name, price, cost, image, stock_quantity, barcode, category)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `).run('Invalid Stock Test', '10.00', '0', 'invalid-stock.jpg', 'not-a-number', 'INVALID_STOCK_123', 'General');
            } catch (error) {
            ...
            this.logIssue(`ERROR SCENARIOS: Failed invalid data format tests - ${error.message}`);
            throw error;
        }
    }
