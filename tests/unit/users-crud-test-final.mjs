...
try {
    ...
    runTest('Role-based access control', () => {
        // Create users with different roles
        const adminResult = db.prepare(
            'INSERT INTO users (name, pin, role) VALUES (?, ?, ?)'
        ).run(`Admin User ${Date.now()}`, generateSecurePin(), 'admin');

        const cashierResult = db.prepare(
            'INSERT INTO users (name, pin, role) VALUES (?, ?, ?)'
        ).run(`Cashier User ${Date.now()}`, generateSecurePin(), 'cashier');

        if (adminResult.lastInsertRowid && cashierResult.lastInsertRowid) {
            console.log('  ✓ Created users with different roles');

            // Cleanup
            db.prepare('DELETE FROM users WHERE id = ?').run(adminResult.lastInsertRowid);
            db.prepare('DELETE FROM users WHERE id = ?').run(cashierResult.lastInsertRowid);
        }
    });

    // TEST 12: Owner user creation
    runTest('Owner user creation', () => {
        const userName = `Owner User ${Date.now()}`;
        const pin = generateSecurePin();

        const result = db.prepare(
            'INSERT INTO users (name, pin, role, is_owner) VALUES (?, ?, ?, ?)'
        ).run(userName, pin, 'admin', 1);
        ...
        const user = db.prepare('SELECT * FROM users WHERE id = ?').get(result.lastInsertRowid);
        if (user.is_owner !== 1) throw new Error('Owner flag not set correctly');
    ...
    fs.writeFileSync('users-crud-test-report-final.json', JSON.stringify(report, null, 2));
    console.log('\n📄 Report saved to: users-crud-test-report-final.json');
